...
function ProductInfo() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`test-1405277720.us-east-1.elb.amazonaws.com/products/${productID}`, {
          headers: {
  ...
  const onSubmithandler = () => {
    ...
    if (token) {
      ...
      fetch(`test-1405277720.us-east-1.elb.amazonaws.com/cart/${productID}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
